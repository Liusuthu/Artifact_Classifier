...
class ConvFeatureVisualization:
    ...
    def visualize(self, conv_layer_indices, layer_idx, image):
        ...
        for i in range(self.conv_output.shape[0]):
            ...
            x = cv2.resize(x, (320, 320), interpolation=cv2.INTER_CUBIC)
...
if __name__ == "__main__":
    ...
    if opt.type == "filter":
        ...
        data_loader = get_data_loader(
            opt.path, "train", image_size=(320, 320), batch_size=8
        )
        labels = []
        features = []
        with torch.no_grad():
        ...
        data_loader = get_data_loader(
            opt.path, "train", image_size=(320, 320), batch_size=16
        )
        labels = []
        features = []
        with torch.no_grad():
